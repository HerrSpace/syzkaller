...
func (ctx *linux) PreviousReleaseTags(commit, compilerFamily string) ([]string, error) {
...
func gitParseReleaseTags(output []byte, includeRC bool) []string {
	var tags []string
	for _, tag_ := range bytes.Split(output, []byte{'\n'}) {
		tag := bytes.TrimLeft(tag_, "linux/")
...
func (ctx *linux) EnvForCommit(compilerFamily, binDir, commit string, kernelConfig []byte) (*BisectEnv, error) {
	...
	err = ctx.git.cherryPickIfMissing("1d489151e9f9d1647110277ff77282fe4d96d09b")
	if err != nil {
		return nil, err
	}
