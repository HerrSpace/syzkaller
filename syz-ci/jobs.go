...
func (jp *JobProcessor) pollRepo(mgr *Manager, URL, branch, reportEmail string) ([]*vcs.Commit, error) {
	...
	repo, err := vcs.NewRepo(mgr.managercfg.TargetOS, mgr.managercfg.Type, dir, mgr.managercfg.KernelTagGlob)
	if err != nil {
...
func (jp *JobProcessor) testPatch(job *Job, mgrcfg *mgrconfig.Config) error {
	...
	repo, err := vcs.NewRepo(mgrcfg.TargetOS, mgrcfg.Type, mgrcfg.KernelSrc, mgrcfg.KernelTagGlob)
	if err != nil {
